...
function backWithError($msg)
{
    ...
    header("Location: IMM_Documentary_Film_Making.php");
    exit();
}
...
$tdata['programme_name'] = 'Certificate in Documentary Film Making';
...
// ✅ success redirect
header("Location: IMM_Documentary_Film_Making_Res.php");
exit();
...
function extraaedgeApi($array_data)
{
    ...
    function debuglog($stringData){
        $logFile = "log/debuglog_Documentary_Film_Making".date("Y-m-d").".txt";
        $fh = fopen($logFile, 'a');
