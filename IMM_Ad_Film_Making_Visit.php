...
function backWithError($msg)
{
    ...
    header("Location: IMM_Ad_Film_Making.php#apply");
    exit();
}
...
$tdata['programme_name'] = 'Certificate in Ad Film Making';
...
// ✅ success redirect
header("Location: IMM_Ad_Film_Making_Res.php");
exit();
...
function extraaedgeApi($array_data)
{
   ...
   function debuglog($stringData){
        $logFile = "log/debuglog_Ad_Film_Making".date("Y-m-d").".txt";
        $fh = fopen($logFile, 'a');
